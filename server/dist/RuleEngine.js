...
// ============================================================
import { getPropertyDef, getRailwayDef, getUtilityDef, calcPropertyRent, calcRailwayRent, calcUtilityRent, canBuildHouse, canSellHouse, getSellHouseValue, CORNER_JAIL, OUTER_RING_OFFSET, JAIL_FINE, MAX_JAIL_TURNS, } from '@monopoly/shared';
...
export class RuleEngine {
    ...
    validateRollDice() {
        ...
        if (this.currentPlayer.isBot)
            return null; // bots always allowed
        return null;
    }
    ...
    // ---- Landing on tile ----
    processLanding(position, extraRentMultiplier = 1) {
        ...
        switch (tile.type) {
            case 'property': {
                ...
                if (owner && owner.id !== player.id) {
                    rentAmount = calcPropertyRent(position, owner.houses[position] || 0, owner, 0, this.effConfig.rentMultiplier);
                    rentTarget = owner.id;
    ...
    validateBuyProperty() {
        ...
        if (this.currentPlayer.cash < tile.price) {
            return '现金不足';
        }
        return null;
    }
    ...
    // ---- Building ----
    validateBuildHouse(tileIndex) {
        if (this.state.phase !== 'rolling' && this.state.phase !== 'awaitEnd')
            return '现在不能建房';
        if (!this.currentPlayer.properties.includes(tileIndex))
            return '你不拥有该地产';
        if (!canBuildHouse(this.currentPlayer, tileIndex))
            return '不满足建房条件';
        return null;
    }
    executeBuildHouse(tileIndex) {
        const player = this.currentPlayer;
    ...
    validateSellHouse(tileIndex) {
        if (this.state.phase !== 'rolling' && this.state.phase !== 'awaitEnd' && this.state.phase !== 'debt') {
            return '现在不能卖房';
        }
        if (!this.currentPlayer.properties.includes(tileIndex))
            return '你不拥有该地产';
        if (!canSellHouse(this.currentPlayer, tileIndex))
            return '不满足卖房条件';
        return null;
    }
    executeSellHouse(tileIndex) {
        const player = this.currentPlayer;
        const value = getSellHouseValue(tileIndex);
        player.houses[tileIndex] = (player.houses[tileIndex] || 0) - 1;
        if (player.houses[tileIndex] <= 0)
            delete player.houses[tileIndex];
        player.cash += value;
    ...
    calcNetWorth(player, stockPrices) {
