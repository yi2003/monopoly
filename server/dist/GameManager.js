...
// ============================================================
import { createTiles, CHANCE_CARDS, COMMUNITY_CHEST_CARDS, SHANGHAI_EXTRA_CHANCE_CARDS, SHANGHAI_EXTRA_COMMUNITY_CHEST_CARDS, TOKYO_EXTRA_CHANCE_CARDS, TOKYO_EXTRA_COMMUNITY_CHEST_CARDS, QUIZ_QUESTIONS, QUIZ_TRIGGER_CHANCE, GO_SALARY, JAIL_FINE, CORNER_GO, CORNER_JAIL, PLAYER_COLORS, } from '@monopoly/shared';
...
export class GameManager {
    ...
    advanceTurn() {
        ...
        let attempts = 0;
        while ((this.state.players[next].status === 'bankrupt' || this.state.players[next].isSpectator) &&
    ...
    // ---- Jail Actions ----
    payJailFine() {
        if (this.currentPlayer.status !== 'jailed')
            return { success: false };
    ...
    useJailCard() {
        if (this.currentPlayer.status !== 'jailed')
            return { success: false };
        if (this.currentPlayer.getOutOfJailCards <= 0)
            return { success: false };
    ...
    drawCard(type) {
        ...
        if (deck.length === 0) {
            ...
            if (type === 'chance')
                this.state.chanceDeck = fresh;
            else
                this.state.communityDeck = fresh;
            deck.length = 0;
            deck.push(...fresh);
        }
    ...
    spinWheel() {
        ...
        const sector = [
            { label: '+$200', effect: { kind: 'cash', amount: 200 } },
            { label: '+$500', effect: { kind: 'cash', amount: 500 } },
            { label: '+$1000', effect: { kind: 'cash', amount: 1000 } },
            { label: '-$100', effect: { kind: 'cash', amount: -100 } },
            { label: '-$300', effect: { kind: 'cash', amount: -300 } },
            { label: '进监狱', effect: { kind: 'jail' } },
            { label: '前进到GO', effect: { kind: 'moveToGO' } },
            { label: '每人付你$50', effect: { kind: 'cashPerPlayer', amount: 50 } },
            { label: '你付每人$50', effect: { kind: 'cashPerPlayer', amount: -50 } },
            { label: '出狱卡', effect: { kind: 'getOutOfJail' } },
            { label: '免费建房', effect: { kind: 'freeHouse' } },
            { label: '科技股×2', effect: { kind: 'freeStock', symbol: 'TECH', shares: 2 } },
            { label: '黄金股×1', effect: { kind: 'freeStock', symbol: 'GOLD', shares: 1 } },
            { label: 'AI股×2', effect: { kind: 'freeStock', symbol: 'AI', shares: 2 } },
        ][sectorIndex];
        // Apply effect
        const player = this.currentPlayer;
        switch (sector.effect.kind) {
            ...
            case 'cashPerPlayer':
                for (const other of this.state.players) {
                    if (other.id === player.id || other.status === 'bankrupt')
                        continue;
            ...
            case 'freeHouse': {
                ...
                for (const idx of player.properties) {
                    ...
                    if (!err) {
                        this.engine.executeBuildHouse(idx);
                        break;
                    }
                }
                break;
            ...
            case 'freeStock': {
                ...
                if (stock) {
                    ...
                    if (existing)
                        existing.shares += sector.effect.shares;
                    else
                        player.stocks.push({ symbol: stock.symbol, shares: sector.effect.shares, avgCost: stock.price });
                }
                break;
            }
        }
    ...
    // ---- High Speed Rail ----
    takeHighSpeedRail(targetTheme) {
        const player = this.currentPlayer;
        const tile = this.state.tiles[player.position];
        if (tile.type !== 'railway')
            return { success: false, error: '当前位置不是铁路' };
    ...
    transferRing(toRing) {
        ...
        const tile = this.state.tiles[player.position];
        if (tile.type !== 'railway')
            return { success: false, error: '只能在铁路站换环' };
        if (player.innerCityRing !== 0)
            return { success: false, error: '不在街道环上' };
        if (player.groundRing === toRing)
            return { success: false, error: '已经在该环上' };
        if (this.state.ringTransferred)
            return { success: false, error: '本回合已换过环' };
    ...
    executeBotAction() {
        if (this.state.phase === 'ended' || this.state.phase === 'lobby')
            return;
        const player = this.currentPlayer;
        if (!player.isBot && !player.autoPilot)
            return;
